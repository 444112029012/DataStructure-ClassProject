...
def del_form_elements(root):
    """創建表單的所有元素"""
    # 名字欄位
    name_label = tk.Label(root, text="欲刪除資料名字:")
    name_label.grid(row=0, column=0, padx=10, pady=5)
    name_entry = tk.Entry(root)
    name_entry.grid(row=0, column=1, padx=10, pady=5)

    # 年齡欄位
    age_label = tk.Label(root, text="欲刪除資料年齡:")
    age_label.grid(row=1, column=0, padx=10, pady=5)
    age_entry = tk.Entry(root)
    age_entry.grid(row=1, column=1, padx=10, pady=5)

    # 性別欄位
    gender_label = tk.Label(root, text="欲刪除資料性別:")
    ...
    # 緊急程度欄位
    urgency_label = tk.Label(root, text="欲刪除資料緊急程度 (數字 1~100):")
...
def Patien_del_access():
    ...
    name_entry, age_entry, gender_var, urgency_entry = del_form_elements(root)
